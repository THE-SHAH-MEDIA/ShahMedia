...
export function AnimatedTestimonials({
  ...
  // Deterministic rotation based on index for SSR compatibility
  const getRotationForIndex = (index: number) => {
    const rotations = [-8, -4, 2, 6, -6, 4, -2, 8, -10, 10];
  ...
  if (!testimonials?.length) {
    return (
      <div className={cn("mx-auto max-w-sm px-4 py-20 font-sans antialiased md:max-w-4xl md:px-8 lg:px-12", className)}>
        <div className="text-gray-400 text-center">Loading testimonials...</div>
      </div>
    );
  }
